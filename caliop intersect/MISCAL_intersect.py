    ...
    while date + datetime.timedelta(1) > iterdate:
        ...
        for sat in res:
            ...
            try:
                ...
                # no TLE on this day, accuracy is bad, just skip
                raise RuntimeError
...
def download_intercepts(terr_url, terr_dir, intercepts, loader=False):
    ...
    larc.cwd(terr_dir)  # TODO account for missing data days
    ...
    for ind, inter in enumerate(intercepts):
        ...
        terr_path = '{dir}/{file}'.format(dir=directory, file=terr_file)
        if not os.path.exists(terr_path) or not os.path.getsize(terr_path):
            with open(terr_path, 'wb') as f:
        ...
        # TODO if time is close to an edge, get the neighbouring granule? some int's have granule off by 5 minutes

        inter_url = 'https://e4ftl01.cr.usgs.gov/MOLT/MOD14.006/{date:%Y.%m.%d}/{gran}'.format(date=day, gran=inter['gran_filename'])
...
if __name__ == "__main__":
    ...
    for day in dates:
        ...
        try:
            ...
            mssg = "--> No TLE entries found on this date, skipping day.\n"
            if debug_flag:
                with open('log.log', 'a') as f:
                    f.write(mssg)
