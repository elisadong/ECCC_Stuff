...
class Plume(object):
    ...
    **Methods**
    ...
    def get_model(self, fst_dir_fire, fst_dir_nofire, var='AF', threshold=4.0):
        ...
        self.Model = _ModelPlume(plm=self, fst_dir_fire=fst_dir_fire, fst_dir_nofire=fst_dir_nofire, var=var, threshold=threshold)
        return
...
class _ModelPlume(object):
    ...
    **Methods**
    ...
    def __init__(self, plm, fst_dir_fire, fst_dir_nofire, var='AF', threshold=4.0):
        ...
        file_time = (plm.datetime if plm.datetime.minute < 30 else plm.datetime + datetime.timedelta(hours=1))
        file_path_fire = "{0}/{1}".format(os.path.abspath(fst_dir_fire), file_time.strftime('%Y%m%d00_0%H'))
        if os.path.isfile(file_path_fire):
            file_path_nofire = "{0}/{1}".format(os.path.abspath(fst_dir_nofire), file_time.strftime('%Y%m%d00_0%H'))
        else:
            ...
            file_path_fire = "{0}/{1}".format(os.path.abspath(fst_dir_fire), file_time.strftime('%Y%m%d12_0%H'))
            file_path_nofire = "{0}/{1}".format(os.path.abspath(fst_dir_nofire), file_time.strftime('%Y%m%d12_0%H'))
        print('Fire Path: {}'.format(file_path_fire))
        print('GEMMACH Path: {}'.format(file_path_nofire))
        ...
        fid_fire = rmn.fstopenall(file_path_fire, rmn.FST_RO)
        fid_nofire = rmn.fstopenall(file_path_nofire, rmn.FST_RO)
        ...
        latarr = plm.latitude
        keylist_fire = rmn.fstinl(fid_fire, nomvar=var)
        keylist_nofire = rmn.fstinl(fid_nofire, nomvar=var)
        ...
        self.dir_fire = os.path.abspath(fst_dir_fire)
        self.dir_nofire = os.path.abspath(fst_dir_nofire)
        ...
        iplist = []
        print(keylist_fire)
        print(keylist_nofire)
        for key_fire, key_nofire in zip(keylist_fire, keylist_nofire):
            meta_fire = rmn.fstprm(key_fire)
            meta_nofire = rmn.fstprm(key_nofire)
            iplist.append(rmn.DecodeIp(meta_fire['ip1'], meta_fire['ip2'], meta_fire['ip3'])[0].v1)

        sorted_keylist_fire = (x for _, x in sorted(zip(iplist, keylist_fire), reverse=True))
        sorted_keylist_nofire = (x for _, x in sorted(zip(iplist, keylist_nofire), reverse=True))

        # --------------------------------------------------------------------------
        # Skip checking the first layer, to avoid the pm2.5 spike near the ground.
        # --------------------------------------------------------------------------

        next(sorted_keylist_fire, None)
        next(sorted_keylist_nofire, None)
        ...
        cur_meta_fire, cur_grid_fire, cur_xypos_fire, cur_val_fire = data_from_key(next(sorted_keylist_fire), fid_fire)
        cur_meta_nofire, cur_grid_nofire, cur_xypos_nofire, cur_val_nofire = data_from_key(next(sorted_keylist_nofire), fid_nofire)
        cur_meta, cur_grid, cur_xypos, cur_val = cur_meta_fire, cur_grid_fire, cur_xypos_fire, cur_val_fire - cur_val_nofire

        load = pyloader.Load_bar(len(keylist_fire) - 2)
        ...
        for progress_ind, (after_key_fire, after_key_nofire) in enumerate(zip(sorted_keylist_fire, sorted_keylist_nofire)):
            ...
            after_meta_fire, after_grid_fire, after_xypos_fire, after_val_fire = data_from_key(after_key_fire, fid_fire)
            after_meta_nofire, after_grid_nofire, after_xypos_nofire, after_val_nofire = data_from_key(after_key_nofire, fid_nofire)
            after_meta, after_grid, after_xypos, after_val = after_meta_fire, after_grid_fire, after_xypos_fire, after_val_fire-after_val_nofire
            ...
            for ind, val in enumerate(cur_val):
                ...
                if (val > before_val[ind] and val > after_val[ind]) and (val >= threshold and val >= self.value[ind]):
                    ...
                    try:
                        # note that the GZ values should be the same for both of them
                        print('Getting GZ at IP1: {}'.format(cur_meta['ip1']))
                        gzkey = rmn.fstinf(fid_fire, nomvar='GZ', ip1=cur_meta['ip1'])['key']
        ...
        rmn.fstcloseall(fid_fire)
        rmn.fstcloseall(fid_nofire)
    ...
    def get_product(self, product):
        ...
        for var in product:
            # TODO for elisa: deal with the is AF case after, but shouldn't ever need to...
            if var != 'AF':
                file_path = "{0}/{1}".format(self.dir_fire, self.name)
