...
# -------------------------------------------------------------------------------
    ...
    # SWITCH TO THIS FUNCTION WHEN TLE FILES SWITCH TO JSON , PASS IN DATE AS DATETIME#
...
# --------------------------------------------------------------------------
    ...
    temp_f.flush()
    fout = subprocess.Popen(['idl', temp_f.name], stdout=subprocess.PIPE).communicate()[0]
    temp_f.close()
