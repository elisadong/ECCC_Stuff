...
def _jd_to_cal(jd):
    ...
    return (yr, mm, dd if jdfrac < 0.5 else dd + 1)
...
def find_overpass(coord, daterange, swath_width=None, sat='Terra', direction='all', tlefile='', show=False):
    ...
    while day <= end:
        ...
        for path in path_list:
            swath = drawswath(path, swath_width)  # if swath_width else path  # TODO doesnt really make sense till bbox is implemented

            if swath:
                swath_coords = [(x[0], x[1]) for x in swath]

                if polygon_check_single(swath_coords, coord, ordered=True):
                    # TODO this way or the block split way?
                    dist_arr = [haver_dist(sat_point[0], sat_point[1], coord[0], coord[1]) for sat_point in path]
