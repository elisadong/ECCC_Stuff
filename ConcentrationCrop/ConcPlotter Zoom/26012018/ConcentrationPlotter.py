...
defaultPathStnd = os.path.join(currentDir,'TestCases/testFiles/GEMMACH_dev/20100710000000/model/')
defaultPathAlt = os.path.join(currentDir,'TestCases/testFiles/GEMMACH_dev/20100710000000/model/')
defaultSave = currentDir
# defaultFile is the name of the FST files to be compared
defaultFile = '2010071000_048'
defaultIp1=76696048
...
def plotConc(concData, lonData, latData, saveLoc, modelRun, prtype, ip1, spc, cmapType, bins, minVal, maxVal, extension, name, removed,buff):
    ...
    # Add subplot
    concData2 = np.zeros([50,50])
    concData2 = concData[30:80,30:80]
    concMap2 = Basemap(projection=prtype, lon_0=-120,lat_0=20,width=max_width/10,height=max_height/10)
    x2,y2 = concMap2(lonData[30:80,30:80],latData[30:80,30:80])
    concMap2.pcolormesh(x2,y2,concData2,cmap=plt.cm.get_cmap(mapColor, mapBins))


    #concMapInset = zoomed_inset_axes(concMap.axes, 5, loc=2)
    #concMapInset.imshow(concData2)
    #concData2 = np.zeros([100,100])
    #ny,nx=concData.shape
    #concData = concData2[50:50+ny,50:50+nx]
    #concMapInset.imshow(concData2)

    #axins = zoomed_inset_axes(ax, 5, loc=1)
    #axins.set_xlim(20,40)
    #axins.set_ylim(20,40)
    #axins.pcolormesh(lonData,latData,concData, extent=(20,40,20,40),interpolation='nearest', origin='lower')
    #mark_inset(ax,axins,loc1=2,loc2=4,fc="none",ec="0.5")

    # Name and save the figure
    #hy = ((os.popen('r.ip1 {}'.format(ip1))).read()).lstrip()
    #plt.title('{}, {} \n  hy: {}, Spc: {}'.format(name,modelRun, hy,spc))
    print('I worked up until saving.')
    plt.show()
