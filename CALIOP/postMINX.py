...
def drawmap(minlon=-180, minlat=-90, maxlon=180, maxlat=90):
    ...
    m.drawparallels([x for x in range(minlat, maxlat + 1, int(round(((maxlat - minlat) / 4) + 0.5)))], labels=[True, False, False, True])
    m.drawmeridians([x for x in range(minlon, maxlon + 1, int(round(((maxlon - minlon) / 4) + 0.5)))], labels=[True, False, False, True])
...
class _Caliop(object):
    ...
    """
    # TODO see CAL_smoke for subtype extraction and better way to do all this probably
    ...
    def __init__(self, plm, intersect, cal_path=os.getcwd(), verbose_error=False, auth=('DicksonN', 'SSHSBench2015!')):
        ...
        if plm_inter_flag:
            ...
            # TODO shouldnt be spoofing time like this
            near_swath = mu.drawswath(list(zip(self.longitude, self.latitude, range(len(self.longitude)))), swath_width=500.)  # TODO fiddle w/ number
